...
public class GoLGrid {
	 ...
	 * 
	 * @param newCells Cell data: entry == 1 -> cell alive (black) / entry == 0 ->
	 *                 cell dead (white)
	 */
	 ...
	 * 
	 * @param probabilityTrue Probability that an element value is true (0.0 - 1.0)
	...
	public int getNumberOfNeighbors(int x, int y) {
		...
		int neighbors = cells[x_m_1][y_m_1] + cells[x][y_m_1] + cells[x_p_1][y_m_1] + cells[x_m_1][y] + cells[x_p_1][y]
				+ cells[x_m_1][y_p_1] + cells[x][y_p_1] + cells[x_p_1][y_p_1];
	...
	private class GoLGridComponent extends JComponent {
		...
		private final Stroke gridStroke = new BasicStroke(1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 10.0f,
				new float[] { 3.0f }, 0.0f);
		...
		protected void paintComponent(Graphics g) {
			...
			for (int y = 0; y < size; y++) {
				for (int x = 0; x < size; x++) {
					...
					g2d.fillRect(x * gridElementPixels + 1, y * gridElementPixels + 1, gridElementPixels - 1,
							gridElementPixels - 1);
				}
			}
